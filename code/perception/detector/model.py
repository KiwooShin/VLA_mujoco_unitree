"""
code/perception/detector/model.py — NX-6 TRAIN (heatmap variant): query-conditioned detection head.
RF-1 split of code/nx6_heatmap_model.py (docs/refactor_plan.md); zero logic change.
...
-> back-project via code.arena.backproject_pixel + code.perception.geometry.cam_to_egocentric
...
from code.perception.geometry import get_ego_intrinsics_rendered, cam_to_egocentric
...
# Both cameras are rendered at FOVY=45deg (code/perception/geometry.py:get_ego_intrinsics_rendered);
...
def decode_single(heat_logit: np.ndarray, dist_resid: np.ndarray, depth_m: np.ndarray,
    ...
    # supervision, which is defined at the GT integer pixel -- see data.py).
